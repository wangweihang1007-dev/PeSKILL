...
本项目是一个本地版 PE/VC/私募项目初筛查重工具。它读取 Excel 历史项目表，将项目追加导入 SQLite，并按标准化后的“项目简称”做精确或包含匹配查询。

第一版只做最小可用功能：Excel 读取、SQLite 存储、项目简称标准化、追加导入去重、结构化查询结果。不使用向量数据库、embedding、RapidFuzz、前端界面或大模型判断。
