---
name: pe-deal-checker-no-dedup
description: Build, inspect, or operate a local PE/VC/私募项目初筛查询工具 that imports every Excel row into SQLite without duplicate filtering and exposes Python functions for project short-name checks. Use when the user asks for PE/私募 deal screening imports that keep duplicate rows, Excel-to-SQLite append-all imports, project short-name normalization, or agent-callable Python functions such as update_deal_database, check_deal_exists, and get_deal_detail.
---
...
Create or maintain a minimal local PE/VC/私募 project checker that keeps duplicate records. Keep the implementation deterministic: Excel input, SQLite storage, normalized project-short-name keys, exact/contains string matching, and structured Python return values.
...
- Use `record_key = normalized_project_short_name`, but do not set a UNIQUE constraint.
- Import in append-all mode: insert every non-empty project row, including rows whose `record_key` already exists.
...
Confirm repeat imports insert duplicate records and that `source_sheet` and `source_row` appear in query results.
