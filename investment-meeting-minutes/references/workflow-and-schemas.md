...
`background_images.json`由脚本直接生成，记录从 BP 的 PPTX/PDF 提取的图片、来源页码和同页文字上下文。生成最终会议纪要时按关键词放入最相关章节；每节最多一张、全文最多六张。不要让模型逐张读取或描述图片。
...
`build_documents.py`读取 `corrected_transcript.txt`、`qa.json`、`minutes.json` 和可选的 `background_images.json`，生成三个DOCX。仅最终会议纪要插入 BP 图片，并将 `qa.json` 追加为“7 访谈记录”。
